 ...
 * @param props.onOpenDirectory - open the session's on-disk artifact directory.
 * @param props.drag - optional draggable-row wiring.
 * @param props.flat - omit the empty status slot in the hierarchy-free flat list.
 * @param props.t - the browser root's locale seat.
 * @returns the session row.
 */
export function SessionNodeItem({ node, currentId, now, onOpen, onRename, onFork, onArchive, onOpenDirectory, drag, flat = false, t }: {
  node: SessionNode
  currentId: string | undefined
  now: number
  ...
  /** Open this session's on-disk artifact directory (row menu action; no dialog). */
  onOpenDirectory: (id: SessionNode['id']) => void
  ...
  const sessionMenuItems = [
    ...
    { id: 'openDirectory', label: t('menu.openSessionDirectory'), icon: <IconFolderOpen16 /> },
        ...
        <span className={css.rowActions}>
          <Menu
            ...
            onSelect={(id) => {
              ...
              if (id === 'openDirectory') onOpenDirectory(node.id)
