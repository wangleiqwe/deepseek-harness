...
`read_image` registers only while a durable `ctx.attachments` service is mounted — without one the deployment cannot commit image bytes, so the tool never appears. Execution gates on the exact routed model's declared input (resolved through `ctx.llm.resolveModelInfo` from the session's latest request header, falling back to agent options). An image-capable route gets the image block itself; a text-only route falls back to the deployed `image-caption` vision route (read from the `image-caption` settings namespace) and returns the image as a text description, keeping a text route's durable history free of image blocks. An unknown or text-only route with no caption configuration gets a refusal result before any filesystem I/O.
...
| `read_image` | `file_path` | Reads a PNG/JPEG/WebP/GIF file through the bounded byte seam, persists it through `ctx.attachments.saveImage`, and returns an image block beside a small metadata envelope on an image-capable route. When the exact routed model cannot carry images, it instead returns the configured image-caption vision route's text description (the bytes are still persisted). It refuses (or falls back) only based on the exact routed model's declared input. |
...
Canonical successes are `read` → `{ path, offset, lines: [{ number, text }], totalLines }`, `read_image` → `{ path, image: { attachmentId, mediaType, bytes, width, height, name? }, caption? }`, `write` → `{ path, operation: 'create' | 'update', before: string | null, after }`, and `edit` → `{ path, before, after }`. The optional `caption` field is present only when a text-only main model triggered the image-caption fallback; the bytes are still durably committed through `attachments.saveImage`, so the `image` reference remains valid either way. Native renderers preserve the line-numbered read and mutation acknowledgements below. `write`/`edit` derive replayable diff-card metadata, and `read` derives a replayable read-card window `{ path, offset, lines, totalLines, lang? }`, from these canonical values; the canonical values themselves are execution-local and are not added to `tool/result`, only the derived presentation metadata is persisted.
...
- **read_image** — validates the argument, extension, attachment availability, and deployment media types before any I/O; resolves the routed model and, for a non-image route, reads the configured `image-caption` vision route (refusing with the original strict message when the settings service, the namespace, `enabled`, or the provider/model is missing); then one `ctx.fs.stat` (recording an `absent` observation for a missing target, like `read`), a bounded `ctx.fs.readBytes` capped at the smaller of `imageLimits.maxImageBytes` and `imageLimits.maxMessageImageBytes` (the result is one message carrying one image), `attachments.saveImage` (content-addressed, so the image block references a durably committed object by the time `tool/result` is appended), and finally `fs/observed`. On a text-only route with a configured vision route, the persisted image is captioned through that route and the result carries the text description instead of an image block. (1 stat.)
...
The model sees the generated [`read`, `read_image`, `write`, and `edit` schemas](../../../docs/tool-catalog.md#deepseek-aidsh-tool-fs), with snake_case arguments. `read_image` appears only while a durable attachment store is mounted; the schema itself is route-independent, and execution gates on the routed model's declared image input — an image-capable route returns the image, a text-only route returns the configured image-caption vision route's text description when one is deployed. Scoped tool restrictions can remove any definition for one agent.
